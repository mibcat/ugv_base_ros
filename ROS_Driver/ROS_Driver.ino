...
void setup() {
  ...
  bool success =
      true;  // Use success to show if the DMP configuration was successful

  // Initialize the DMP. initializeDMP is a weak function. You can overwrite it
  // if you want to e.g. to change the sample rate
  success &= (myICM.initializeDMP() == ICM_20948_Stat_Ok);

  // Enable the DMP orientation sensor
  success &= (myICM.enableDMPSensor(INV_ICM20948_SENSOR_ORIENTATION) ==
              ICM_20948_Stat_Ok);

  success &= (myICM.enableDMPSensor(INV_ICM20948_SENSOR_RAW_GYROSCOPE) ==
              ICM_20948_Stat_Ok);
  success &= (myICM.enableDMPSensor(INV_ICM20948_SENSOR_RAW_ACCELEROMETER) ==
              ICM_20948_Stat_Ok);
  // +
  success &=
  ...
  // success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Quat9, 3) ==
  // ICM_20948_Stat_Ok); // Set to the maximum

  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Accel, 3) ==
              ICM_20948_Stat_Ok);  // Set to the maximum
  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Gyro, 3) ==
              ICM_20948_Stat_Ok);  // Set to the maximum

  // +
  success &= (myICM.setDMPODRrate(DMP_ODR_Reg_Cpass, 3) == ICM_20948_Stat_Ok);
  success &=
      (myICM.setDMPODRrate(DMP_ODR_Reg_Cpass_Calibr, 3) == ICM_20948_Stat_Ok);
...
void loop() {
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) ||
    ...
    //   // Scale to +/- 1
    //   q1 = ((double)data.Quat9.Data.Q1) / 1073741824.0; // Convert to double.
    //   Divide by 2^30 q2 = ((double)data.Quat9.Data.Q2) / 1073741824.0; //
    //   Convert to double. Divide by 2^30 q3 = ((double)data.Quat9.Data.Q3) /
    //   1073741824.0; // Convert to double. Divide by 2^30 q0 = sqrt(1.0 - ((q1
    //   * q1) + (q2 * q2) + (q3 * q3)));
